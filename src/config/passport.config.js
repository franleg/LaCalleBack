...
import { userService, bookingService } from '../services/services.js';
...
const initializePassport = () => {
    ...
    passport.use('register', new LocalStrategy(
        ...
        async (req, email, password, done) => {
            try {
                const { first_name, last_name, address, phone, dni, age } = req.body;
                if (!first_name || !last_name || !age || !address || !phone || !dni || !email || !password) return done(null, false, {message: 'Valores incompletos'});
                let exists = await userService.getByEmail(email);
                if (exists) return done(null, false, {message: 'El usuario ya existe'});
                let booking = await bookingService.save();
